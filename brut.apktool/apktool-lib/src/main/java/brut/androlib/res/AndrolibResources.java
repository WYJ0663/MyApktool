...
final public class AndrolibResources {
    ...
    private void aapt2Package(File apkFile, File manifest, File resDir, File rawDir, File assetDir, File[] include,
        ...
        File resourcesZip = aapt2PackageCompile(apkFile, manifest, resDir, rawDir, assetDir, include,
                cmd, customAapt);

        aapt2PackageLink(apkFile, manifest, resDir, rawDir, assetDir, include,
                cmd, customAapt, resourcesZip);
    }

    private File aapt2PackageCompile(File apkFile, File manifest, File resDir, File rawDir, File assetDir, File[] include,
                                     List<String> cmd, boolean customAapt)
            throws AndrolibException {
    ...
    private void aapt2PackageLink(File apkFile, File manifest, File resDir, File rawDir, File assetDir, File[] include,
                                  List<String> cmd, boolean customAapt, File resourcesZip)
            throws AndrolibException {
    ...
    private void aapt1Package(File apkFile, File manifest, File resDir, File rawDir, File assetDir, File[] include,
        ...
        if (apkOptions != null && apkOptions.RPath != null) {
            cmd.add("-J");
            cmd.add(apkOptions.RPath);
        }
